...
from .models import Article, BigCategory, Category, Tag,Resource
...
class ResourceView(generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['is_in'] = True if self.request.user.is_authenticated() else False
        return context
def UploadView(request):
    if request.user.is_authenticated():
        ...
        print(title,summary,file)
        try:
            Resource.objects.create(author=request.user,title=title,doc=file,summary=summary)
            print('upload  success')
            return redirect('/category/resource//')
        except:
            print('upload error')
            return render_to_response('404.html')

def DownloadView(request,resource_id):
    resource = Resource.objects.get(id=resource_id)
    file_url = resource.doc.path
    file_name = resource.get_title()
